...
class class_order_update_database extends class_database {
    ...
    public function update_order_stats($order_id, $stat){
        ...
        try{
            ...
            $this->query("COMMIT");
            echo json_encode(array("stat" => $stat , "result" => "success"));
            exit;
        }catch(Exception $error){
            echo "Failed: " . $error->getMessage();
            $this->query('ROLLBACK');
            echo json_encode(array("stat" => $stat , "result" => "failed"));
            exit;
        }
    ...
    public function prepare_transaction($tranaction_id){
        try{
            ...
            $this->query("COMMIT");
            echo json_encode(array("stat" => "prepared" , "result" => "success"));
        }catch(Exception $error){
            echo "Failed: " . $error->getMessage();
            $this->query('ROLLBACK');
            echo json_encode(array("stat" => "prepared" , "result" => "failed"));
        }
    }
}
