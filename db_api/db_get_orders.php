<?php
    ...
    class class_order_info extends class_database {
        ...
        public function get_orders_info($status){
            $get_orders_info = ("SELECT odr.order_id, item.item_name, vari.variation_name, odr.order_qty, odr.order_price,odr.date_requested, item_img.item_img, username.username, customer.customer_id, odr.customer_id
            FROM tbl_order odr 
            ...
            LEFT JOIN tbl_customer customer ON odr.customer_id = customer.customer_id
...
<!-- SELECT item.item_name, vari.variation_name, odr.order_qty, odr.order_price,odr.date_requested, item_img.item_img
FROM tbl_order odr 
LEFT JOIN tbl_variation vari ON vari.variation_id = odr.variation_id 
LEFT JOIN tbl_item item ON item.item_id = vari.item_id
LEFT JOIN tbl_item_img item_img ON item.item_id = item_img.item_id AND item_img.is_variant = 0
GROUP BY odr.orderID -->
